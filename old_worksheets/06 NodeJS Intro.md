...
Navigate to the `06_nodejs_intro/` directory and then install the package.
...
1. locate the `input` variable declaration (just inside the `do` loop)
    1. define it as a block-scoped variable by replacing the `var` with `let`, what effect does this have? Can you explain why this is the case?
    2. modify the script so that it still works (keep the `let` variable declaration). Hint: think about the variable _scope_, you will need to move the variable declaration.
    3. substitute a constant by substituting `const` for `let`, what effect does this have?
...
3. Items are added to the array using its `push()` method.
    1. substute the [unshift](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/unshift) method. How does this change the script?
...
2. If an exception is _thrown_ the execution moves to the `catch{}` block.
    1. the error object thrown will be passed as the parameter.
